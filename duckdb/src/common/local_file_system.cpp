...
void TouchBytes(void* _buffer, size_t size){
	volatile int result = 1;
	for(size_t i = 0; i < size; i++) {
		result |= ((u_int8_t *)_buffer)[i];
	}
	// cout << "touching Bytes: " << reinterpret_cast<void*>(_buffer) << endl;
}
void LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	while(true) {
		...
		if(retry > 10) {
			if (bytes_read == -1) {
				throw IOException("Check: Could not read from file \"%s\": %s", handle.path, strerror(errno));
			}
			if (bytes_read != nr_bytes) {
				throw IOException("Check: Could not read all bytes from file \"%s\": wanted=%lld read=%lld", handle.path, nr_bytes, bytes_read);
			}
		}
		retry ++;
	}
...
int64_t LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	int64_t bytes_read;
	while(true) {
		bytes_read = read(fd, buffer, nr_bytes);
		if (bytes_read == -1) {
			TouchBytes(buffer, nr_bytes);
			// throw IOException("Check2: Could not read from file \"%s\": %s", handle.path, strerror(errno));
		} else {
			return bytes_read;
		}
		if(retry > 10000) {
